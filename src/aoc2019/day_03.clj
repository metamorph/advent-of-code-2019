  ...
  (let [targets (set targets)]
    (loop [state     {}
      ...
      (if (nil? p)
        state
        (if (targets p)
          (recur (assoc state p steps) pts (inc steps))
          (recur state pts (inc steps)))))))

(defn solve-2
  "Solve problem 2 - sort by combined distance per intersection of wires."
  [wire-1 wire-2]
  (let [intersections (map first (rank wire-1 wire-2))
        steps-1 (steps-to (unpack [0 0] wire-1) intersections)
        steps-2 (steps-to (unpack [0 0] wire-2) intersections)]
    (sort (vals (merge-with + steps-1 steps-2)))))
